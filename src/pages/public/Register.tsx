  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8080/api/v1/auth/register', {
        ...
        // It tells the browser to accept and store the HTTP-Only cookies Go sends back.
        withCredentials: true 
      });

      console.log("Registration Successful!", response.data);
      
      // Route them to their specific dashboard based on what they selected
      ...
      // Display the error message sent from Go (e.g., "Email already in use")
      if (error.response && error.response.data.error) {
        setErrors(prev => ({ ...prev, email: error.response.data.error }));
      } else {
        setErrors(prev => ({ ...prev, email: "Server error. Please try again later." }));
      }
    } finally {
      setIsLoading(false);
    }
